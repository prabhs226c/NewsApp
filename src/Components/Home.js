...
//            fetch('http://localhost:2260/admin/users',{
    ...
    deleteUser = (e)=>{
            ...
            if(agree === true)
            {
                fetch("https://backend-newz.herokuapp.com/api/admin/deleteUser/"+userId,{
                    method:"post",
                    mode:"cors",
                    headers: {
                        'Content-Type': 'application/json'
        ...
        if(response.statusCode == 401)
        {
            ...
            response.json().then(json => {
                ...
                const users = json.map((_value,_key)=>{
                    return(<tr key={"homrTable"+_value.id}><td>{_value.first_name}</td><td>{_value.last_name}</td><td>{_value.email_id}</td><td><a userid={_value.id} onClick={this.deleteUser} href={"#"}>Delete User </a></td></tr>)
                })
